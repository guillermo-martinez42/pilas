...
// Arma un banco a partir de filas [número, pregunta, r1, r2, r3, r4]. Lo usan
// el CSV y el formulario de la maestra: una sola regla para los dos caminos.
...
export function buildBank(tema, rows, errors = []) {
  ...
  rows.forEach((r, i) => {
    const line = i + 1;
    const [num = '', text_ = '', ...rest] = r.map((v) => String(v ?? '').trim());
