...
export class Room {
  ...
  reset() {
    ...
    this.count = Math.min(10, this.maxCount());
    this.qi = 0;
    this.questions = [];
    this.history = [];
    this.deadline = null;
  ...
  maxCount() {
    return Math.max(MIN_COUNT, Math.min(MAX_COUNT, this.bank()?.questions.length ?? MAX_COUNT));
  }
  ...
  setFilter(f) { this.filter = String(f ?? 'Todos'); this.changed(); }

  setCount(n) {
    this.count = Math.max(MIN_COUNT, Math.min(this.maxCount(), Number(n) || MIN_COUNT));
    this.changed();
  }
  ...
  startGame() {
    ...
    this.questions = shuffle(bank.questions).slice(0, this.count);
    this.count = this.questions.length;   // si el banco tiene menos, manda el banco
    this.qi = 0;
    this.history = [];
    for (const p of this.players.values()) { p.score = 0; p.picks = []; }
  ...
  startQuestion() {
    this.stopTimer();
    this.step = 'question';
    this.deadline = Date.now() + QUESTION_MS;
    this.voicePlay = null;
    this.voiceDone = false;
    this._timer = setTimeout(() => this.toReveal(), QUESTION_MS + GRACE_MS);
    this.changed();
  }
  ...
  advance() {
    switch (this.step) {
      case 'topic': this.step = 'count'; this.changed(); break;
      case 'count': this.step = 'lobby'; this.changed(); break;
  ...
  back() {
    if (this.step === 'count') this.step = 'topic';
    else if (this.step === 'lobby') this.step = 'count';
  ...
  hostView() {
    const v = this.base();
    const bank = this.bank();
    v.filters = ['Todos', ...new Set(this.banks.map((b) => b.cat))];
    v.filter = this.filter;
    v.topic = this.topic;
    v.topicLabel = bank?.label ?? '';
    v.maxCount = this.maxCount();
    v.topics = this.banks
      .filter((b) => this.filter === 'Todos' || b.cat === this.filter)
      .map((b) => ({
        id: b.id,
        label: b.label,
        glyph: b.glyph,
        meta: b.questions.length + ' preguntas · ' + b.grade,
      }));
    v.seats = [...this.players.values()]
      .sort((a, b) => a.seat - b.seat)
