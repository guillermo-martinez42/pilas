...
export class Room {
  ...
  hostView() {
    ...
    // En el paso 1 van las preguntas en el formato del asistente, para poder
    // editarlas en el teléfono después de subir el CSV: la correcta primero.
    if (this.step === 'topic' && bank) {
      v.questions = bank.questions.map((q) => ({
        seconds: q.seconds ?? 20,
        text: q.text,
        answers: q.open ? [] : [q.opts[q.correct], ...Object.entries(q.opts).filter(([k]) => k !== q.correct).map(([, t]) => t)],
      }));
    }
    v.seats = [...this.players.values()]
      .sort((a, b) => a.seat - b.seat)
